...
export const TrackletStitcher: React.FC<TrackletStitcherProps> = ({
  roster, assignments, ignoredTracks, players, trackingData, onClose, onUpdate
}) => {
                ...
                {group.assignedPlayers.map(p => {
                  ...
                  return (
                    <div 
                      ...
                      className="absolute top-1 bottom-1 bg-blue-500 rounded text-xs px-1 whitespace-nowrap overflow-hidden cursor-move border border-blue-400 shadow flex items-center"
                      style={{ left: `${left}%`, width: `${width}%`, backgroundColor: group.rosterProfile.color }}
                      title={`Track ${p.id}: Frames ${p.firstFrame}-${p.lastFrame}`}
                    >
                      {p.id}
                    </div>
                  );
